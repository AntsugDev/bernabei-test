...
class AuthCustAuthenticator  extends AbstractAuthenticator implements AuthenticationEntryPointInterface
{
    use TargetPathTrait;
        ...
        if(count($form) > 0) {
            ...
            return new SelfValidatingPassport(
                new UserBadge($form['email'], function () use ($form,$password,$request) {
                    try {
                        $this->validate($form['email'], $password);

                        if (is_null($this->user->getUsername())){
